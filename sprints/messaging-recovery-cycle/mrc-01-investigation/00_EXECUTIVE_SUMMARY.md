...
- Trixus backend answers on `localhost:3001/api/health`; Evolution container can reach it through `host.docker.internal:3001`.
...
- Prisma migration status reports a drift in database `trixus`: local migration `20260806120000_messaging_core_completion` is not applied, while database migration `20260730132000_redis_bullmq_outbox` is not present locally.
- A manual query against `trixus` confirmed `messages` is missing the new message core columns, while `trixus_0802` contains them. This environment split is a major forensic risk.
