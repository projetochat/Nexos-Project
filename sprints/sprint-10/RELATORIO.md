...
Contact detail e Conversation detail no dominio oficial Trixus API.
...
- Verify inicial em `trixus_0801`: PASS.
...
- Tags usam Trixus API para listar, criar, aplicar e remover.
...
- `trixus_1000`: drop/create, `prisma migrate deploy`, seed e smoke SQL de duplicata global Quick Reply.
- `trixus_0802`: `prisma migrate deploy` aplicado sem reset, seed idempotente executado.
- `trixus_0801`: migration aplicada para suite regressiva automatizada.

## Validacao automatizada

`bun run verify` em `trixus_0801`: PASS.
...
## Smoke fisico em trixus_0802

Backend real iniciado contra `trixus_0802`.
...
Seed idempotente executado em `trixus_0802` sem reset. Confirmado:

- `admin@trixus.app`: `tenant_admin`, `chat.tags.use`, `chat.tags.manage`,
  `chat.quick_replies.read`, `chat.quick_replies.manage`.
- `atendente@trixus.app`: `agent`, `chat.tags.use`, `chat.quick_replies.read`, sem manage de Tags/Quick Replies.
- Preset frontend de atendente em `/perfis` inclui `chat.tags.use`.

### Conversation create

`POST /api/conversations` validado fisicamente contra backend real em `trixus_0802` como admin:
...
`["trixus", "tags"]`. Catalogo e associacao foram separados: catalogo vem de Tags ativas do tenant; Contact
...
Smoke fisico em `trixus_0802` confirmou criar Tag, listar por admin/agente, associar ao Contact, consultar
...
`/mensagens-rapidas` usa `quickReplyApi.list/create/update/archive`. Smoke fisico em `trixus_0802` confirmou
...
- `git diff --check`: PASS.
- `bun run verify` #1 em `trixus_0801` com Redis local: PASS.
- `bun run verify` #2 em `trixus_0801` com Redis local: PASS.

### Physical tests

Smoke fisico REST em `trixus_0802`:
...
| M104 | admin role audit                     | `tenant_admin` ativo em `homologacao`                                                  | Seed/audit `trixus_0802`                                        | PASS    |
...
| M107 | seed permissions corrected           | Seed idempotente aplicado; roles existentes atualizados                                | `bun run backend:prisma:seed` em `trixus_0802`                  | PASS    |
| M108 | admin create Conversation            | Conversation criada                                                                    | HTTP 201, protocol `000011`                                     | PASS    |
| M109 | admin Tag manage                     | Admin cria/arquiva Tag                                                                 | Smoke REST `trixus_0802`                                        | PASS    |
| M110 | admin Quick Reply manage             | Admin cria/edita/arquiva Quick Reply                                                   | Smoke REST e e2e                                                | PASS    |
| M111 | Tag catalog API audit                | Catalogo vem da Trixus API                                                             | `/etiquetas` usa `crmApi`                                       | PASS    |
| M112 | Contact Tag modal audit              | Modal lista catalogo do tenant                                                         | `crmApi.listTags`, key `["trixus","tags"]`                      | PASS    |
| M113 | catalog vs assignment fix            | Catalogo separado de Contact tags                                                      | Contact detail + assign/remove endpoints                        | PASS    |
| M114 | Tag cache fix                        | Invalidacao canonica adicionada                                                        | `["trixus","tags"]` e contact queries                           | PASS    |
...
| M120 | Quick Reply create physical          | Criacao via Trixus API                                                                 | Smoke REST `quickReplyCreated=true`                             | PASS    |
...
| M131 | seed idempotency                     | Seed reaplicado sem reset                                                              | `trixus_0802`                                                   | PASS    |
| M132 | trixus_0802 audit                    | Admin/agente auditados                                                                 | SQL/Prisma audit                                                | PASS    |
...
| M137 | verify #1                            | Suite completa passou                                                                  | `bun run verify` em `trixus_0801`                               | PASS    |
...
Conclusao: inbound WhatsApp -> Trixus nao pode ser considerado preservado ate prova fisica sem esses erros.
...
- Regressao automatizada final aprovada: `bun run verify` em `trixus_0801` com Redis local, 20 arquivos
...
- Quick Replies via Trixus API: PASS
