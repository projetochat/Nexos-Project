...
export function canCopyMessage(message: ApiMessage) {
  return message.type === "text" || (!!message.media_data && (!message.media_data.state || message.media_data.state === "ready"));
}

/** Send a new copy; never changes the source message or carries its quoted-message IDs. */
export async function sendMessageCopy(message: ApiMessage, destination: string, clientMessageId: string) {
  if (!canCopyMessage(message)) throw new Error("Esta mensagem ainda não está disponível para envio.");
  if (message.type === "text") return messageApi.sendText(destination, message.content, clientMessageId);
