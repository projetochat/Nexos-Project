...
describe("dashboard calendar periods", () => {
  it("starts this week on Sunday and ends last week on Saturday", () => {
    ...
    expect(datesForOperationalPeriod("previous_week")).toEqual({ start: "2026-09-06", end: "2026-09-12" });
    expect(datesForOperationalPeriod("previous_year")).toEqual({ start: "2025-01-01", end: "2025-12-31" });
  });
  ...
  it("keeps Sunday in the new week across a year boundary", () => {
    ...
    expect(datesForOperationalPeriod("previous_week")).toEqual({ start: "2025-12-28", end: "2026-01-03" });
  });
});
