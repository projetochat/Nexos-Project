    ...
    await db.$transaction(
      async (tx) => {
        const tenantA = await tx.tenant.create({
          data: { name: "Verification A", slug: "verify-a-" + randomUUID() },
        });
        const tenantB = await tx.tenant.create({
          data: { name: "Verification B", slug: "verify-b-" + randomUUID() },
        });
        ...
        if ((await mobile.list(userA)).length !== 1 || (await mobile.list(userB)).length !== 0)
          throw Error("Tenant isolation failed");
        await mobile.save({ ...item, title: "Mobile edit" } as never, userA);
        if ((await desktop.list(userA))[0].title !== "Mobile edit")
          throw Error("Mobile edit not persisted");
        await desktop.save(item as never, userB);
        if ((await tx.schedule.count({ where: { id: item.id } })) !== 2)
          throw Error("Tenant key collision");
        await mobile.remove(item.id, userA);
        if ((await desktop.list(userA)).length !== 0 || (await desktop.list(userB)).length !== 1)
          throw Error("Delete isolation failed");
        results.schedules =
          "create/read/edit/delete persisted, shared both directions, tenant isolation verified";
        const connection = await tx.messagingConnection.findFirst({
          where: { archivedAt: null, providerType: "EVOLUTION" },
        });
        const membership =
          connection &&
        ...
        if (!connection || !membership)
          throw Error("No local campaign fixture connection/membership");
        await tx.messagingConnection.update({
          where: { id: connection.id },
          data: { status: "CONNECTED" },
        });
        const persistence = new Proxy(tx, {
          get(target, key) {
            if (key === "$transaction")
              return (queries: Promise<unknown>[]) => Promise.all(queries);
        ...
        if (
          !list.items.some((item) => item.id === campaign.id) ||
          !(await tx.campaign.findUnique({ where: { id: campaign.id } }))
        )
          throw Error("Campaign not persisted/listed");
