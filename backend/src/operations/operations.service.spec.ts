...
describe("dashboard week ranges", () => {
  it.each([
  ...
  ] as const)("uses Sunday boundaries for %s in the company timezone", async (period, start, end) => {
    vi.useFakeTimers();
    ...
    const result = await service.dashboard({ tenantId: "tenant-a", roleKey: "agent", connectionIds: ["vocical"] } as never, { period });
    expect(result.range).toEqual({ start, end });
    expect(metrics.snapshot).toHaveBeenNthCalledWith(1, "tenant-a", {
      start: new Date(start), end: new Date(end),
    }, { period, allowedConnectionIds: ["vocical"] });
    expect(metrics.chartData).toHaveBeenCalledWith("tenant-a", expect.anything(), { period, allowedConnectionIds: ["vocical"] });
