...
export function connectionAccess(current: Pick<AuthenticatedUser, "roleKey" | "connectionIds">): Prisma.ConversationWhereInput {
  return current.roleKey === "tenant_admin" ? {} : { connectionId: { in: current.connectionIds ?? [] } };
}

export function connectionIdAccess(current: Pick<AuthenticatedUser, "roleKey" | "connectionIds">): Prisma.MessagingConnectionWhereInput {
